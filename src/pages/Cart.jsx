...
function Cart() {
   const { cartItems, cartTotal, shipping, tax, orderTotal } = useSelector((state) => state.cartState)
         ...
         <div className="mt-8 flex flex-col lg:flex-row gap-x-12">
            <div className="grow">
               {cartItems && cartItems.map(item => {
                  return (
                     <div key={item.cartID} className="flex flex-col gap-y-4 mb-12 border-b border-base-300 last:border-b-0 pb-6 sm:flex-row">
                        <img src={item.image} alt={`${item.title} image`} className="h-24 w-24 object-cover sm:h-32 sm:w-32 rounded-lg" />
                        <div className="sm:ml-16 sm:w-48">
                           ...
                           <div className="mt-4 flex text-sm items-center gap-2">
                              Color: <span className="badge badge-sm" style={{ background: item.productColor }}></span>
                           </div>
                        </div>
                        <div className="form-control max-w-xs sm:ml-12 sm:items-center">
                           ...
                           <select
                              ...
                              onChange={(e) => dispatch(editItem({ cartID: item.cartID, amount: parseInt(e.target.value) }))}
                           >
                              {generateAmountOptions(20)}
                           </select>
                           <button onClick={() => dispatch(removeItem({ cartID: item.cartID }))} className="mt-2 link link-primary link-hover text-sm text-left">remove</button>
                        </div>
